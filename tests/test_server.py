...
class TestDashboard:
	...
	async def test_dashboard_shows_config(self, client):
	...
	async def test_dashboard_shows_stats(self, client):
	...
	async def test_dashboard_shows_users(self, client):
...
class TestSubmitWork:
	...
	async def test_submit_work_requires_post(self, client):
		"""Test that GET requests are not allowed on submit_work."""
		resp = await client.get("/submit_work")
		assert resp.status == 405  # Method Not Allowed

	async def test_submit_work_rejects_invalid_json(self, client):
		"""Test that invalid JSON is rejected."""
		resp = await client.post("/submit_work", data="not json")
		assert resp.status == 400

	async def test_submit_work_rejects_bad_credentials(self, client):
	...
	async def test_submit_work_accepts_valid_credentials(self, client):
	...
	async def test_submit_work_rejects_bad_hash_length(self, client):
	...
	async def test_submit_work_validates_distinguished_point(self, client):
...
class TestIntegration:
	...
	async def test_user_stats_update_after_submission(self, client, test_db):
	...
	async def test_dashboard_reflects_database_state(self, client, test_db):
